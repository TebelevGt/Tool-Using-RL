...
- Генерируем датасеты ```python generate_curriculum_train_dataset.py```  для обучения по расписанию  
- ```python generate_train_test_dataset.py``` для генерации датасета для валидации  
в папке notebooks забираем и через kaggle/colab запускаем следующие ноутбуки:  
- Запускаем ноутбук 1 для обучения модели  
- С помощью ноутбука 2 для измерения качества  

# 2. Оптимизация агентных способностей Qwen 2.5 с помощью GRPO в среде ToolEnv
...
![alt text](results/grpo_training_dynamics.png)

![alt text](results/pendolf_evaluation_results.png)


## 2.5 Система штрафов при обучении агента
